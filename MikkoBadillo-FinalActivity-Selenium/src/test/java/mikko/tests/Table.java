...
public class Table extends TestBase{
	...
	public void tablePagination() throws InterruptedException
	{
		...
		int tablerowCount = driver.findElements(By.xpath("//tbody[@id='myTable']//tr[@style='display: table-row;']")).size();
	...
	public void dataSearch() throws InterruptedException
	{
		...
		if (tableBody.findElements(By.xpath(".//td[text()='" + dataText + "']")).size() > 0) 
			...
			System.out.println(tableBody.findElements(By.xpath(".//td[text()='" + dataText + "']")).size()+" " + dataText + " is displayed in the column ");
			} 
		else 
			{
	...
	public void tableFilterDemo() throws InterruptedException
	{
		...
		Assert.assertTrue(driver.findElement(By.xpath("//tr[@data-status='pagado']")).isDisplayed(), "green not displayed");
		...
		Assert.assertTrue(driver.findElement(By.xpath("//tr[@data-status='pendiente']")).isDisplayed(), "orange not displayed");
		...
		Assert.assertTrue(driver.findElement(By.xpath("//tr[@data-status='cancelado']")).isDisplayed(), "red not displayed");
	...
	public void tableSortAndSearch() throws InterruptedException
	{
		...
		List<Object> positionOriginalList = positionList.stream().map(s->s.getText()).collect(Collectors.toList());
		...
		if (tableBody.findElements(By.xpath(".//td[text()='" + searchText + "']")).size() > 0) 
			...
			System.out.println(tableBody.findElements(By.xpath(".//td[text()='" + searchText + "']")).size()+" " + searchText + " is displayed in the column ");
			} 
		else 
			{
